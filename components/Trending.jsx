...
const Trending = ({ post }) => {
    ...
    const viewableItemsChanges = ({ viewableItemss }) => {
        if (viewableItemss?.length > 0) {
            setActiveItem(viewableItemss[0].key);
        }
    }

    return (
        <FlatList data={post} keyExtractor={(item) => item.$id} renderItem={({ item }) => (
            <TrendingItem activeItem={activeItem} item={item} />
        )}
            onViewableItemsChanged={viewableItemsChanges}
            viewabilityConfig={{
                itemVisiblePercentThreshold: 70
            }} contentOffset={{ x: 170 }} horizontal />
    )
}
